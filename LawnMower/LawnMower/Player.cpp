...
#include "Player.h"
#include "Level1.h"
#include <algorithm> // Necess�rio para std::min e std::max
#include <cmath>
#include <sstream>
// ---------------------------------------------------------------------------------
...
void Player::OnCollision(Object * obj)
{
    ...
    if (obj->Type() == VILLAIN) {
        ...
        // 2. Calcular o centro do jogador e do vil�o
        ...
        // 3. Calcular o vetor de dire��o (do vil�o para o jogador)
        float dirX = playerCenterX - villainCenterX;
        float dirY = playerCenterY - villainCenterY;

        // 4. Normalizar o vetor de dire��o
    ...
    if (obj->Type() == WALL) {
        ...
        // Usamos static_cast porque sabemos que a geometria � um ret�ngulo (Rect)
        Rect* wallBBox = static_cast<Rect*>(obj->BBox());

        // 2. Calcular a sobreposi��o (penetra��o) nos eixos X e Y
        ...
        // 3. Determinar o lado da colis�o e resolver com base na menor penetra��o
        if (overlapX < overlapY)
        {
            // Colis�o horizontal � menor

            // Se o centro do jogador est� � esquerda do centro da parede
            if (playerBBox->X() + (playerBBox->Right() - playerBBox->Left()) / 2.0f < wallBBox->X() + (wallBBox->Right() - wallBBox->Left()) / 2.0f) {
                // Colis�o pela esquerda, empurra para a esquerda
                Translate(-overlapX, 0);
            }
            else {
                // Colis�o pela direita, empurra para a direita
                Translate(overlapX, 0);
            }
            velX = 0;
        }
        ...
        {
            // Colis�o vertical � menor ou igual

            // Se o centro do jogador est� acima do centro da parede (considerando Y crescente para baixo)
            if (playerBBox->Y() + (playerBBox->Bottom() - playerBBox->Top()) / 2.0f < wallBBox->Y() + (wallBBox->Bottom() - wallBBox->Top()) / 2.0f) {
                // Colis�o por cima, empurra para cima
                Translate(0, -overlapY);
            }
            else {
                // Colis�o por baixo, empurra para baixo
                Translate(0, overlapY);
            }
            velY = 0;
        }  
